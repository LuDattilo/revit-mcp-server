...
export function registerBatchModifyViewRangeTool(server) {
    server.tool("batch_modify_view_range", "Batch modify View Range settings (Top, Cut Plane, Bottom, View Depth offsets) on multiple plan views at once. Only specified offsets are changed; others remain unchanged.\n\nGUIDANCE:\n- Set cut plane: modify cut plane height for multiple plan views at once\n- Adjust bottom: change bottom clip offset for views\n- Standardize: apply same view range settings across all floor plans\n\nTIPS:\n- View range only applies to plan views (floor plans, ceiling plans)\n- Heights are relative to the associated level\n- Use get_current_view_info to check current view range settings\n- Changes affect element visibility in plan views", {
        ...
        try {
            const params = { viewIds: args.viewIds };
            if (args.topOffsetMm !== undefined)
                params.topOffsetMm = args.topOffsetMm;
            if (args.cutPlaneOffsetMm !== undefined)
                params.cutPlaneOffsetMm = args.cutPlaneOffsetMm;
            if (args.bottomOffsetMm !== undefined)
                params.bottomOffsetMm = args.bottomOffsetMm;
            if (args.viewDepthOffsetMm !== undefined)
                params.viewDepthOffsetMm = args.viewDepthOffsetMm;
