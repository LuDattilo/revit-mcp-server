...
export function registerCreateGridTool(server) {
    server.tool("create_grid", "Create grid lines (linear or arc) with optional naming pattern.", {
        xCount: z
            .number()
            .int()
            .positive()
        ...
        try {
            ...
            return rawToolError("create_grid", `Create grid failed: ${errorMessage(error)}`);
        }
    });
}
