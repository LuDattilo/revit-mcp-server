...
export function registerRenameFamiliesTool(server) {
    server.tool("rename_families", "Rename loaded families with find/replace or prefix/suffix.", {
        operation: z
        ...
        try {
            ...
            return rawToolResponse("rename_families", response);
        }
        catch (error) {
            return rawToolError("rename_families", `Rename families failed: ${errorMessage(error)}`);
        }
    });
}
