...
export function registerCreateFloorTool(server) {
    server.tool("create_floor", "Create a floor element from boundary points on a level.", {
        ...
        roomId: z
            .number()
            .optional()
            .describe("Room element ID to derive floor boundary from (alternative to boundaryPoints)"),
        floorTypeName: z
            .string()
            .optional()
        ...
        try {
            ...
            return rawToolError("create_floor", `Create floor failed: ${errorMessage(error)}`);
        }
    });
}
